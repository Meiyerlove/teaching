		...
		<%@include file="common.jsp" %><div class="main-content">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<select style="height:30px;outline:none;" onchange="switchTClass();" name="classid">
						<c:forEach items="${tclassList}" var="tclass">
							<option value="${tclass.id}"<c:if test="${tclass.id==classid}">selected</c:if>>${tclass.name}</option>
						</c:forEach>
					</select>
					...
					<table class="table table-bordered table-striped" id="example-2">
						...
						<c:forEach items="${studentList}" var="student">
							<tr>
								...
								<c:if test="${student.status==1}"><td>在校</td></c:if>
								<c:if test="${student.status==2}"><td>离校</td></c:if>
								<td>
								    <a href="javascript:lookAbsenceDetail('${student.sid}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										查看考勤
									</a>
								</td>
							</tr>
...
function lookAbsenceDetail(sid){
				$("#sid").val(sid);
				document.forms[0].action = "FindStudentHistoryAbsenceDetailInformation.action";
				document.forms[0].submit();
		}

function switchTClass(){
				document.forms[0].action = "FindStudentHistoryAbsenceInformation.action";
				document.forms[0].submit();
		}
