	...
	<div class="page-container"><!-- add class "sidebar-collapsed" to close sidebar by default, "chat-visible" to make chat appear always -->
		...
		<%@include file="common.jsp" %><div class="main-content">
			...
			<div class="panel panel-default">
				...
				<div style="height:30px;background-color:#5CA323;color:#FFF;line-height:30px;text-align:center;cursor:pointer;float:right;margin-left:20px;">
						<select onchange="changeclass();" id="selectclassid" style="background-color:#5CA323;border:none;outline:none;color:#FFF;">
							<option value="0">班级筛选</option>
								<c:forEach items="${classList}" var="class">
									<option value="${class.id}" <c:if test="${class.id==classid}">selected</c:if>>${class.name}</option>
								</c:forEach>
						</select>
				</div>
				</c:if>								
				<div onclick="sort_subject()" style="width:100px;height:30px;background-color:#5CA323;color:#FFF;line-height:30px;text-align:center;cursor:pointer;float:left;">科目排序</div>
				</div>
