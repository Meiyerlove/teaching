						...
						<tbody class="middle-align">
						  <c:forEach items="${classList}" var="cla" varStatus="status">
							<tr>
								...
								<td>${cla.name}</td>
								<td>
									<a href="javascript:toStudentListLiuji('${cla.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										分班
									</a>
								</td>
							</tr>
...
function toStudentListLiuji(classid){
                document.getElementById("classid").value = classid;
				document.forms[0].action = "ToStudentListLiuji.action";
				document.forms[0].submit();	
		}
function logout(){
			if (confirm("您确定要退出吗?")){
