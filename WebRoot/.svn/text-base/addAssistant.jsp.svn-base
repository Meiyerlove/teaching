						...
						<div class="panel-body">						
							<form role="form" class="form-horizontal">			
								<div class="form-group">
									...
									<div class="col-sm-10">	
										 <input type="text" name="assistantname" class="form-control" id="assistantname"/>
										 <div id="assistantnameDiv" style="display: inline;"></div>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
									<label class="col-sm-2 control-label">辅导员工号:</label>
									<div class="col-sm-10">
									  <input type="text" name="assistantaid" id="assistantaid" class="form-control" placeholder="请输入数字">
									   <div id="assistantaidDiv" style="display: inline;"></div>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
									<label class="col-sm-2 control-label">辅导员性别:</label>								
									 <div class="col-sm-10">
										<select id="assistantsex" name="assistantsex" class="form-control"> 
										  <option value="">----</option>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
									  <input type="text" id="oph" name="oph" class="form-control"/>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
...
function logout(){
     ...
     var flag1=true;
	$(function(){
         $("#assistantname").bind("blur",function(){
          var assistantname=$("#assistantname").val().trim();
          if(assistantname!=""){
            $.post("CheckAssistantname.action",{assistantname:assistantname},
              function(data){
                 if(data==true){
                   $("#assistantnameDiv").html("<font color='red'>你输入的辅导员姓名已存在，请重新输入!</font>");
                   flag=false;
                }else{
                   $("#assistantnameDiv").empty();
                   flag=true;
                 }    
               });
             }
    ...
    $("#assistantaid").bind("blur",function(){
         var regex="^[0-9]*$";
         var teachertid=$("#assistantaid").val();
         if(teachertid!="" && !teachertid.match(regex)){
             $("#assistantaidDiv").html("<font color='red'>请输入的编号有误，请重新输入!</font>"); 
             flag1=false;
             return false;
         }
           if($("#assistantaid").val()!=""){
              $("#assistantaidDiv").empty();
              $.post("Checkassistantaid.action",{assistantaid:$("#assistantaid").val()},
               function(data){
                 if(data==true){
                  $("#assistantaidDiv").html("<font color='red'>你输入的班级编号已存在,请重新输入!</font>");
                  flag1=false;
                 }else{
                   flag1=true;
                 }
	...
	function doTclasstAdd() {
		...
		if(null == assistantname || assistantname == '')
		{
			alert('辅导员名称不能为空!');
			return false;
		} 
		var assistantaid = document.getElementById("assistantaid").value;
		if(null == assistantaid || assistantaid == '')
		{
			alert('辅导员编号不能为空!');
			return false;
		}
		var assistantsex = document.getElementById("assistantsex").value;
		if(null == assistantsex || assistantsex == '')
		{
			alert('辅导员性别不能为空！');
			return false;
		}
        ...
        if(confirm("你确定要提交申请吗")){
		 document.forms[0].action = "DoassistantAdd.action";
