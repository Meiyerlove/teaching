		...
		<%@include file="common2.jsp" %><div class="main-content">
			...
			<div class="panel panel-default">
				...
				<c:if test="${not empty noticeList}">
					...
					<table id="noticeallid" style="display:none;"></table>	
				</div>
			</div>
...
function toNoticeAdd(){
				...
				window.location="ToNoticeAdd.action";
				//document.forms[0].submit();	
		}
function returnNotice(id){
      $.post("Check_Notice.action",{id:id},
			function(data){
			var obj=data.split(":");
				if(data=="false"){
				alert("发送通知已经超过10分钟不能进行撤回操作!");	
				  return false;				
			    }else{
...
<script type="text/javascript" src="flexigrid_my/jquery-1.3.2.min.js"></script>
		...
		<script type="text/javascript" src="flexigrid_my/noticeallList.js"></script>
</html>