					...
					<table class="table table-bordered table-striped" id="example-2">
						...
						<c:forEach items="${scoreList}" var="score" varStatus="status">
							<tr>
								...
								<td>
								<c:if test="${score.examtype==1}">
								   正考 
								</c:if>
								<c:if test="${score.examtype==2}">
								   正考-补做 
								</c:if>
								<c:if test="${score.examtype==3}">
								   正考-补考
								</c:if>
								<c:if test="${score.examtype==4}">
								   正考-补考-补做 
								</c:if>
								<c:if test="${score.examtype==5}">
								   正考-重修
								</c:if>
								<c:if test="${score.examtype==6}">
								   正考-重修-补做
								</c:if>
								<c:if test="${score.examtype==7}">
								   毕前考
								</c:if>
								<c:if test="${score.examtype==8}">
								 毕前考-补做  
								</c:if>
								<c:if test="${score.examtype==9}">
								   其他
								</c:if>
								<c:if test="${score.examtype==10}">
								  其他-补做 
								</c:if>
							   </td>
								<td>${score.result_}</td>
