	...
	<div class="page-container"><!-- add class "sidebar-collapsed" to close sidebar by default, "chat-visible" to make chat appear always -->
		...
		<%@include file="common.jsp" %><div class="main-content">
			...
			<div class="panel panel-default">
				...
				<div class="panel-heading">
					<div>
							<select id="kebiao" name="kebiao" onchange="selectKebiao();">
										<option value="1" <c:if test="${kebiao==1}">selected</c:if>>辅导员课表</option>					
										<option value="2" <c:if test="${kebiao==2}">selected</c:if>>班级课表</option>
							</select>
							<c:if test="${not empty kechengVersionList}">
								<select id="version" name="version" onchange="findFudaoyuanKebiaoListByVersionAndClass();" style="float:right;">
							...
							<c:if test="${not empty tclassList&&kebiao==2}">
								<select id="class" name="class" onchange="findFudaoyuanKebiaoListByVersionAndClass();" style="float:right;margin-right:10px;">
									<c:forEach items="${tclassList}" var="class_">
										<option value="${class_.id}" <c:if test="${class_.id==classId}">selected</c:if>>${class_.name}</option>
									</c:forEach>						
								</select>
							</c:if>
					</div>
							  ...
							  <c:forEach items="${kebiaoList}" var="kebiao" varStatus="status">
								<tr>
								    ...
								    <c:forEach items="${kebiao}" var="kecheng" varStatus="status">
									   <td>${kecheng.subjectname}<br/>${kecheng.teacher}<br/>${kecheng.classroom}</td>
									</c:forEach>
								</tr>
								</c:forEach>
...
function findFudaoyuanKebiaoListByVersionAndClass(){
				document.forms[0].action = "FindFudaoyuanKebiaoListByVersionAndClass.action";
				document.forms[0].submit();	
		}
function selectKebiao(){
				var kebiao=$("#kebiao").val();
				if(kebiao==1){
				document.forms[0].action = "FindFudaoyuanKebiaoListByVersion.action";
				document.forms[0].submit();
				}else{
				document.forms[0].action = "FindFudaoyuanKebiaoListByVersionAndClass.action";
				document.forms[0].submit();
