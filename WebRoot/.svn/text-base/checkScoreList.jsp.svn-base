						...
						<tbody class="middle-align">
						  <c:forEach items="${examList}" var="exam" varStatus="status">
							<tr>
								...
								<td>
									<c:if test="${exam.displaystatus==5}">
									<a href="javascript:toCheckScore('${exam.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										<h5>审核成绩</h5>
									</a>
									</c:if>
								</td>
...
function toCheckScore(examid){
                document.getElementById("examid").value = examid;
				document.forms[0].action = "ToCheckScore.action";
				document.forms[0].submit();	
		}
