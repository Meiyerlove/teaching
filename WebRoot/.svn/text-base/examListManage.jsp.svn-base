		...
		<%@include file="common.jsp" %><div class="main-content">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">我的考试安排数据</h3>
				</div>
				
				<div class="panel-body" style="overflow:scroll;height:550px;">
					<table class="table table-bordered table-striped" id="example-2"  style="width:1100px;">
						<thead>
							<tr>
						...
						<tbody class="middle-align">
						  <c:forEach items="${examList}" var="exam" varStatus="status">
							<tr>
								...
								<td>
								   <c:if test="${exam.displaystatus < 4}">
									<a href="javascript:toScoreInput('${exam.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										<h5>录入成绩</h5>
									</a>
									</c:if>
									<c:if test="${exam.displaystatus == 4}">
									<a href="javascript:cancelRelease('${exam.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										<h5>撤回发布</h5>
									</a>
									</c:if>
								</td>
...
function toScoreInput(examid){
                document.getElementById("examid").value = examid;
				document.forms[0].action = "ToScoreInput.action";
				document.forms[0].submit();	
		}
function cancelRelease(examid){
                document.getElementById("examid").value = examid;
				document.forms[0].action = "CancelRelease.action";
				document.forms[0].submit();	
		}		
