...
<table>
	<tr><td></td><td></td>
        <td></td>
        <td></td>
        <td align="center"><input name="new" type="button" value="新增合同" onclick="addContract()"/></td>
	</tr>
    <tr><td></td>
        <td></td>
        <td><br></br></td>
	...
	<tr>
	    ...
	    <th>合同编号</th>
	    <th>学生</th>
		<th>有效课时数<img src="images/up.jpg" onclick="sortContractByTotalUp()"/>&nbsp;<img src="images/down.jpg" onclick="sortContractByTotalDown()"/></th>
		<th>合同金额</th>
		<th>签约日期</th>
		<th>合同期限<img src="images/up.jpg" onclick="sortContractByDateUp()"/>&nbsp;<img src="images/down.jpg" onclick="sortContractByDateDown()"/></th>
		<th>合同付款状态</th>
		<th>合同类型</th>
		<th>顾问</th>
		<th>合同照片</th>
	...
	<s:iterator id="list" value="conList" status="sta">
		<tr>
            ...
            <td width="7%" align="center">${list.contractNO}</td>
			<td width="7%" align="center">${list.emplName}</td>
			<td width="7%" align="center">${list.totalLessonAmount}</td>
			<td width="7%" align="center">${list.amount}</td>
			<td width="7%" align="center">${list.contractDateFormat}</td>
			<td width="7%" align="center">${list.expireDateFormat}</td>
			<td width="7%" align="center">${list.contractPayStatus}</td>
			<td width="7%" align="center">${list.contractType}</td>
			<td width="7%" align="center">${list.salesConsultant}</td>
			<td width="8%" align="center">
			     <s:if test="1 == ${list.isExistImage}">
			     <a href="${pageContext.request.contextPath}/Contract/${list.contractNO}.jpg" target="_blank">
                     <img src="${pageContext.request.contextPath}/images/touxiang1.jpg" />
                 </a>
                 </s:if>
                 <s:else>暂无照片</s:else>
                </td>
			<td width="7%" align="center">${list.userName}</td>
			<td width="10%" align="center">
			<input name="delete" type="button" value="编辑" onclick="editEmployee('${list.emplID}')" disabled="true"/>
			<input name="delete" type="button" value="删除" onclick="deleteContract('${list.contractNO}')"/></td>
		</tr>
	</s:iterator>
</table>
</s:form>
        ...
        function detailEmployee(emplID){
         ...
         function editEmployee(emplID){
          ...
          function addContract(){
    	    document.forms[0].action = "AddContract.action";
			document.forms[0].target = "main";
			document.forms[0].submit();
         }
          ...
          function deleteContract(contractNO){
             if(confirm("确定删除合同吗")){
                     document.forms[0].action = "DeleteContract.action?contractNO="+contractNO;
			         document.forms[0].target = "main";
			         document.forms[0].submit();
                 }else{
                        return false;
