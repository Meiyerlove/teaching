	...
	<div class="page-container">
		...
		<%@include file="common.jsp" %><div class="main-content">
			...
			<div class="row">
			 ...
			 <c:if test="${tuser.userType==2}">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">调课申请列表</h3>
				</div>
						...
						<tbody class="middle-align">
						  <c:forEach items="${adjustList}" var="adjust" varStatus="status">
							<tr>
								...
								<td>${adjust.reason}</td>
								<td>${adjust.edureason}</td>
								<%--<td>
								 <c:if test="${adjust.status == 2}">
								  等待审批
								</c:if> --%>
								<%-- <c:if test="${adjust.status == 2}">
								  审批通过
								</c:if>
								<c:if test="${adjust.status == 3}">
								  审批未通过
								</c:if>
								</td> --%>
								<td>
								<c:if test="${adjust.status == 2}">
								<input type="button" value="同意" class="btn btn-secondary btn-sm btn-icon icon-left" onclick="openCommentDiv(${adjust.id},'agree');" >
							    <input type="button" value="不同意" class="btn btn-secondary btn-sm btn-icon icon-left"  onclick="openCommentDiv(${adjust.id},'noagree');">	
		   ...
		   <div id="commentdiv" style="display:none;z-index:11;position:absolute;left:400px;top:350px;">
			<form id='myfrm' name='myfrm'  method="post">
			<div style="text-align:left;margin-left:10px;">
         	  <textarea placeholder="请输入审批理由:" cols=50 rows=10 style="text-align:left;padding-left:3px;border-radius: 5px;" name="content" id="content"></textarea>
         	  <br/>
         	  <input type="button" value="确认"  onclick="approveAdjust()"/>&nbsp;
         	  <input type="button" value="取消" onclick="closeCommentDiv()"/>
