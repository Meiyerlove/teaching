							...
							<form role="form" class="form-horizontal">			
								<div class="form-group">
									...
									<div class="col-sm-10">	
										 ...
										 <div id="numberDiv" style="display: inline;"></div>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
									   <input type="text" name="classvid" id="classvid" class="form-control" />   
									    <div id="classvidDiv" style="display: inline;"></div>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
									...
									<div class="col-sm-10">
									   <select class="form-control" id="assistantid" name="assistantid">
...
function logout(){
	...
	$(function(){
         ...
         var classname=$("#classname").val().replace(/\s+/g,"");
          if($("#classname").val()!=""){
            $.post("CheckClassname.action",{classname:classname},
              function(data){
                 if(data==true){
                   $("#numberDiv").html("<font color='red'>你输入的班级已存在，请重新输入!</font>");
                   flag=false;
                 }else{
                   $("#numberDiv").empty();
                   flag=true;
        ...
        $("#classvid").bind("blur",function(){
         var regex="^[0-9]*$";
         var classid=$("#classvid").val();
         if(classid!="" && !classid.match(regex)){
             $("#classvidDiv").html("<font color='red'>你输入的数字编号不规范，请重新输入!</font>"); 
               flag1=false;  
               return false;
          }
           if(classid!=""){
              $("#classvidDiv").empty();
              $.post("CheckClasscid.action",{classcid:$("#classvid").val()},
               function(data){
                 if(data==true){
                  $("#classvidDiv").html("<font color='red'>你输入的班级编号已存在，请重新输入!</font>");
                  flag1=false;
                 }else{
                  flag1=true;
                 }
	...
	function doTclasstAdd() {
		...
		if(null == gradename || gradename == '')
		{
			alert('年级名称不能为空！');
			return false;
		}
		var classvid = document.getElementById("classvid").value;
		if(null == classvid || classvid == '')
		{
			alert('班级编号名称不能为空！');
			return false;
		}
		...
		if(null == majorid || majorid == '')
		{
			alert('班级专业不能为空！');
			return false;
		} */
         if(confirm("你确定要提交申请吗")){
		 document.forms[0].action = "DoTclasstAdd.action"
