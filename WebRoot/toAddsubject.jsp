							...
							<form role="form" class="form-horizontal">			
								<div class="form-group">
									...
									<div class="col-sm-10">	
										 ...
										 <div id="subjectnameDiv" style="display: inline;"></div>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
									  <input type="text" name="subid" id="subid" class="form-control">
									   <div id="subidDiv" style="display: inline;"></div>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
									<div class="col-sm-10">
										...
										<input type="button" class="form-control" style="margin-left:600px;width:80px;background:#e4393c;color:white;margin-top:-32px;" id="field-6" value="新增" onclick="doSubjectAdd();">
									</div>
								</div>
...
function logout(){
	...
	$(function(){
         $("#subjectname").bind("blur",function(){
          ...
          if(subjectname!=""){
            $.post("CheckSubjectname.action",{subjectname:subjectname},
              function(data){
                 if(data==true){
                   $("#subjectnameDiv").html("<font color='red'>你输入的课程名称已存在，请重新输入!</font>");
                     flag=false;
                }else{
                   $("#subjectnameDiv").empty();
                    flag =true; 
                 }    
               });
             }
    ...
    $("#subid").bind("blur",function(){
    	    ...
    	    if(subid!=""){
              $.post("CheckSubjectid.action",{subid:subid},
               function(data){
                 if(data==true){
                  $("#subidDiv").html("<font color='red'>你输入的课程编号已存在,请重新输入!</font>");
                  flag1 =false;
                 }else{
                  flag1 =true;
                  $("#subidDiv").empty();
	...
	function doSubjectAdd() {
		...
		if(null == subid || subid == '')
		{
		 ...
		 document.forms[0].action = "DoSubjectAdd.action";
