						...
						<tbody class="middle-align">
						  <c:forEach items="${roleList}" var="role" varStatus="status">
							<tr>
								...
								<td>${role.desc}</td>
								<td>
									<a href="javascript:toAssignRight('${role.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">
										分配权限
									</a>
								</td>
							</tr>
...
function toAssignRight(roleid){
                document.getElementById("roleid").value = roleid;
				document.forms[0].action = "ToRoleAssignRight.action";
				document.forms[0].submit();	
		}
function logout(){
			if (confirm("您确定要退出吗?")){
