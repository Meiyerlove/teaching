							...
							<form role="form" class="form-horizontal">			
								<div class="form-group">
									...
									<div class="col-sm-10">	
									     ...
									     <input type="hidden" name="subjectid_" id="subjectid_"/> 
										 <input type="text" name="subjectname" class="form-control" id="subjectname" value="${subject.name}"/> 
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
									  <input type="text" name="subid" id="subid" class="form-control" value="${subject.subid }">
									  <div id="subidDiv"></div>
									</div>
								</div>
								<div class="form-group-separator"></div>
								<div class="form-group">
									<div class="col-sm-10">
										...
										<input type="button" class="form-control" style="margin-left:600px;width:80px;background:#e4393c;color:white;margin-top:-32px;" id="field-6" value="修改" onclick="doUpdatesubject();">
									</div>
								</div>
...
<script type="text/javascript">
         ...
         var flag1=true;
        $(function(){
         $("#subjectname").bind("blur",function(){
          var subjectname=$("#subjectname").val().trim();
          var subjectid=$("#subjectid").val();
          if(subjectname!=""){
              $.post("CheckExistSubjectName.action",{subjectname:subjectname,subjectid:subjectid},
               function(data){
                 if(data==true){
                    flag=false;
                    $("#subjectDiv").html("<font color='red'>该课程名称已经存在，请重新输入!</font>");
                 }else{
                     flag=true;
                    $("#subjectDiv").empty();
                 }
          ...
          $("#subid").bind("blur",function(){
             var subjectid=$("#subjectid").val();
             var subid=$("#subid").val().trim();
             if(subid!=""){
               $.post("CheckExistSubid.action",{subid:subid,subjectid:subjectid},
                   function(data){
                     if(data==true){
                      flag1=false;
                      $("#subidDiv").html("<font color='red'>你输入的课程编号已存在,请重新输入!");
                     }else{
                        flag1=true;
                        $("#subidDiv").empty();
                     }
...
function logout(){
	...
	function doUpdatesubject() {
		 ...
		 document.forms[0].action = "DoSubjectUpdate.action"
