						...
						<div class="icon-collection">
							<section id="web-application">
								<div class="fontawesome-icon-list">
									<div class="fa-hover"><i class="fa fa-plus-square" onclick="addtclass()"></i> <span>增加班级</span></div>
								</div>
							</section>
						</div>
					</div>
				...
				<div class="col-sm-12">		
					<div class="panel panel-default">
						...
						<tbody class="middle-align">
						  <c:forEach items="${listnewclasslist}" var="cla" varStatus="status">
							<tr>
								...
								<td>
									<a href="javascript:toClassEnter('${cla.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">进入班级</a>
									<a href="javascript:toClassUpdate('${cla.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">修改班级</a>
									<a href="javascript:toClassDelete('${cla.id}');" class="btn btn-secondary btn-sm btn-icon icon-left">删除</a>
								</td>
							</tr>
							</c:forEach>
					      ...
					      <input type="hidden" name="tclassid" id="tclassid"/>
					      <input type="hidden" name="_tclassid" id="_tclassid"/>
							</form>
						</div>
					</div>
...
function addtclass(){
          document.forms[0].action = "AddTclass.action";
		  document.forms[0].submit();	
		}
function toClassEnter(clid){
     $("#tclassid").val(clid);
     document.forms[0].action="ToClassEnter.action";
     document.forms[0].submit();
}
function toClassUpdate(clid){
      $("#tclassid").val(clid);
      document.forms[0].action = "ToClassUpdate.action";
	  document.forms[0].submit();	

}
function toClassDelete(clid){
      alert("暂时不做删除业务逻辑");
      return;
   /*   $("#tclassid").val(clid);
     $("#_tclassid").val(0);
